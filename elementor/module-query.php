...
class Module_Query_Base {
	...
	public function register_ajax_actions( $ajax_manager ) {
		$ajax_manager->register_ajax_action( 'maxcoach_elementor_autocomplete_search', [
			$this,
			'autocomplete_search',
		] );
		$ajax_manager->register_ajax_action( 'maxcoach_elementor_autocomplete_render', [
			$this,
			'autocomplete_render',
		] );
	}
	...
	public function autocomplete_search( array $data ) {
		...
		switch ( $query_data['object'] ) {
			...
			case self::QUERY_OBJECT_AUTHOR:
				...
				break;
			default:
				$results = apply_filters( 'maxcoach_elementor_autocomplete_get_' . $query_data['filter_type'], $results, $data );
				break;
		}
	...
	public function autocomplete_render( $request ) {
		...
		switch ( $query_data['object'] ) {
			...
			case self::QUERY_OBJECT_USER:
				...
				break;
			default:
				$results = apply_filters( "maxcoach_elementor_autocomplete_get_value_titles/{$query_data['filter_type']}", $results, $request );
		}
	...
	private function get_term_name( $term, $display, $request, $filter_name = 'get_autocomplete' ) {
		...
		switch ( $display ) {
			...
			case 'minimal':
				...
				break;
			default:
				$text = apply_filters( "maxcoach_elementor_query_{$filter_name}_display_{$display}", $term_name, $request );
				break;
		}
	...
	private function autocomplete_query_data( $data ) {
		...
		if ( in_array( $autocomplete['object'], self::$supported_objects_for_query, true ) ) {
			...
			$autocomplete = wp_parse_args( $autocomplete, $defaults );
